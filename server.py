...
class Server():
    ...
    def create_update_message(self):
        return tchat.info_message_encode(self.server_name, self.max_clients, len(self.connected_sockets) - 1, list(self.sockets_names.values()), False)
    ...
    def stop_server(self):
        object = tchat.general_message_encode(tchat.CONSOLE_INFO, tchat.CONSOLE_SEPERATOR, f"Server has been closed, for clients use /disc to disconnect...", tchat.TEXT_COLOR_YELLOW)
    ...
    def remove_client(self, client):
        ...
        object = tchat.general_message_encode(tchat.CONSOLE_INFO, tchat.CONSOLE_SEPERATOR, f"{name} has left the server...", tchat.TEXT_COLOR_YELLOW)
        self.message_broadcast(object)

    def message_broadcast(self, message_object, type=tchat.MESSAGE_GENERAL):
        ...
        if type == tchat.MESSAGE_INFO:
            self.gui.win_draw_sidebar(message_object)
        elif type == tchat.MESSAGE_GENERAL:
            message = tchat.message_decode(message_object)
